function [whole_image black_color white_color] = image_series_yu(threed_image,maxxx,comp_rows,comp_columns)
if nargin<2
maxxx=max(max(max(threed_image)));
end
...
%               spacing (optional)  Number of pixels between images
...
%               )
threed_image=single(threed_image);
...
if isempty(comp_rows) && isempty(comp_columns)
    ...
    while (comp_columns / comp_rows) > 2
...
%    figure();
    if exist('threshold','var') && ~isempty(threshold)
        imagesc(whole_image,[0 maxxx]);
%        imagesc(whole_image,[white_color max(threshold)]);
%        imagesc(whole_image,[0 6e6]);
    else
%        imagesc(whole_image);
        imagesc(whole_image,[0 maxxx]);
%        imagesc(whole_image,[0 6e6]);
